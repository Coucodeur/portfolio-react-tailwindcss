...
const Projectcard = ({ projet }) => {
  const { title, status, description, technologies, github, link } = projet;
  ...
  return (
    <div className="projetContainer opacity-90 w-11/12 max-w-96 p-1 flex flex-col gap-4 rounded-sm hover:bg-textcolor hover:bg-opacity-5 hover:opacity-100 hover:shadow-2xl bg-textcolor bg-opacity-5 md:bg-transparent md:p-3">
      <h1 className="projetTitle text-2xl text-importanttextcolor">{title}</h1>
      <div>
        {status === 'En cours' ? (
          <span className="projetStatus text-importantcolor">{status}</span>
        ) : (
          <span className="projetStatus text-red-600">{status}</span>
        )}
      </div>
      <div className="projetDescription">{description}</div>
